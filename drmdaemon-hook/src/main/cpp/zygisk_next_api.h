...
struct ZygiskNextAPI {
  ...
  // Do plt hook at symbol specified by the param `symbol` of library specified
  // by the param `base_addr` The plt address of `symbol` in the library will be
  // replaced with hook_handler, and its original value will be put to the
  // address specified by `origianl` (can be null). You can use this api to do
  // caller-oriented hook If you want to unhook, please call this function with
  // hook_handler = original If hook succeed, returns ZN_SUCCESS, otherwise
  // ZN_FAILED
  int (*pltHook)(void *base_addr, const char *symbol, void *hook_handler,
                 void **original);

  // Do inline hook at the address specified by `target`, replace it with a new
  // function specified by `addr`, and the param `original` receives the address
  // of original function. You can use this api to achieve a global hook in
  // current process. In the current implementation , an address can only hook
  // once, so the module can't hook an address which is already hooked by an
  // another module, except that the module unhooked it. If hooking succeed,
  // returns ZN_SUCCESS, otherwise ZN_FAILED
  ...
  // `path` is required, which specifies the path of library to resolve. It can
  // be an absolute path or just the file name of library, e.g.
  // /system/lib64/libc.so or libc.so . If `base_addr` is non-zero, it will be
  // used as the base address of the library. Otherwise, Zygisk Next will try to
  // find out the base address of the specified library in this process. If
  ...
  // Walk through the symbol table of the library, the callback will receive the
  // name, the address, and the size of each symbol. Returning false in the
  // callback means stop the walking.
...
struct ZygiskNextCompanionModule {
  ...
  // establish a connection with your companion module, i.e. `connectCompanion`
  // is called. The `fd` param will be a unix sock stream file descriptor.
